...
//					printf("audio format =%d,output format=%d\n",pFrameAudio->format, outSampleFmt);
				...
				if( AV_NeedTranscode(AudioFmt.pFrame->format) )
				{
...
/*
		            audio_chunk = (uint8_t*)soundframe.data;  
		            audio_len = soundframe.linesize;  
		            audio_pos = soundframe.data;  
...
//						printf("audio format =%d--------------------\n",pFrameAudio->format);
					PACKETINFO info;
					info.iSize=AudioFmt.pFrame->linesize[0];
					memcpy(info.data,AudioFmt.pFrame->data[0],info.iSize);
					g_VideoState.queueAudioFrame.Stream_Push(info);
				}
			}
			av_free_packet(&packet);
