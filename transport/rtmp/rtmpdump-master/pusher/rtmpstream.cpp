...
bool CRTMPStream::SendSpsPps(LPRTMPMetadata lpMetaData)  
{
    if(lpMetaData == NULL)  
    {  
        return false;  
...
{  
    ...
    m_h264Desc.fs.open(pFileName, std::ios::in|std::ios::binary);
    H264Framer_Init(m_h264Desc);
...
/*    memset(&metaData,0,sizeof(RTMPMetadata));  
    ...
    h264_decode_sps(metaData.Sps+4,metaData.nSpsLen-4,width,height,fps);  
    metaData.nWidth = width;  
    metaData.nHeight = height;  
    metaData.nFrameRate = fps;  
    printf("sps width=%d height=%d fps=%d \n", width, height, fps);
...
*/
    ...
    {  
        ...
        // 发送H264数据帧   
        if( !SendH264Packet(naluUnit.data,naluUnit.size,bKeyframe,tick) )
            break; 
...
{  
    int ret = H264Framer_GetFrame(m_h264Desc, (char*)nalu.data, sizeof(nalu.data));
    if( ret < 0 )
    {
        ret = H264Framer_ReadData(m_h264Desc);
        if( ret < 0 ) 
        {
            printf("H264Framer_ReadData error \n");
            return FALSE;
        }
        
        ret = H264Framer_GetFrame(m_h264Desc, (char*)nalu.data, sizeof(nalu.data));
        if( ret < 0 ) 
        {
            printf("H264Framer_GetFrame error \n");
            return FALSE;
        }
    }
    nalu.size = ret;
    if( nalu.data[2] == 0x1 )
        nalu.type = nalu.data[3] & 0x1F;
    else
