...
{  
    OpenBitstreamFile(filePath);//打开264文件，并将文件指针赋给bits,在此修改文件名实现打开别的264文件。  
    NALU_t *n;  
    char* nalu_payload;    
    char sendbuf[1500];  
...
//        dump(n);//输出NALU长度和TYPE  
    //（1）一个NALU就是一个RTP包的情况： RTP_FIXED_HEADER（12字节）  + NALU_HEADER（1字节） + EBPS  
    	...
    	{
    		h264dec_seq_parameter_set(n->buf+1, n->len - 1, &sps);

            if( sps.vui_parameters.timing_info_present_flag )
            {
                framerate = sps.vui_parameters.time_scale / sps.vui_parameters.num_units_in_tick;
...
{  
    ...
    int bytes=0;  
    float framerate=15;  
    unsigned int timestamp_increse = 90000/25, ts_current = 0;  
    timestamp_increse=(unsigned int)(90000.0 / framerate); //+0.5);  //时间戳，H264的视频设置成90000    
    ...
    //（1）一个NALU就是一个RTP包的情况： RTP_FIXED_HEADER（12字节）  + NALU_HEADER（1字节） + EBPS  
        ...
        {     
            ...
            bytes=n->len + 12 ;  //获得sendbuf的长度,为nalu的长度（包含NALU头但除去起始前缀）加上rtp_header的固定长度12字节  
            if( callback )
            	callback( sendbuf, bytes, pUserData);
            //  Sleep(100);
        }
        else if(n->len > 1400)  //这里就要分成多个RTP包发送了。  
        {  
