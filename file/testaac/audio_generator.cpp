...
void *audio_generator_alloc(int bits_per_sample, int channels, int nb_samples)
{
	audio_generator_tag_t *inst = new audio_generator_tag_t;

    /* init signal generator */
    inst->t     = 0;
    inst->tincr = 2 * M_PI * 110.0 / c->sample_rate;
    /* increment frequency by 110 Hz per second */
    inst->tincr2 = 2 * M_PI * 110.0 / c->sample_rate / c->sample_rate;
...
int audio_generator_get_audio_frame(void* handle, const char** frame, int *Length)
{
    ...
    for (j = 0; j < nb_samples; j++) 
    {
	    ...
	    {
	    	inst->audio_frame.append((const char*)&v, bits_per_sample/8);
	    }
