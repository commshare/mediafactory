...
typedef struct get_bit_context{
    uint8_t *buf;         /*ÖžÏòSPS start*/
    int      buf_size;    /*SPS ³€¶È*/
    int     bit_pos;      /*bitÒÑ¶ÁÈ¡Î»ÖÃ*/
    int     total_bit;    /*bit×Ü³€¶È*/
    int     cur_bit_pos;  /*µ±Ç°¶ÁÈ¡Î»ÖÃ*/
}get_bit_context;
 ...
 * parameters:
 *   *buf :     buf ptr, ÐèÍ¬²œ00 00 00 01 X7ºóŽ«Èë
 *   *sps_ptr:  spsÖžÕë£¬±£ŽæSPSÐÅÏ¢
 * 
 * return:
 *   0K 0£¬ERR -1
 * byproduct:
 ...
 *   none
 ***********************************************************************/
int h264dec_seq_parameter_set(void *buf, int buflength, SPS *sps_ptr);
 ...
 * parameters:
 *   *buf :     buf ptr, ÐèÍ¬²œ00 00 00 01 X8ºóŽ«Èë
 *   *pps_ptr:  ppsÖžÕë£¬±£ŽæPPSÐÅÏ¢
 * 
 * return:
 *   0K 0£¬ERR -1
 * byproduct:
 *   none
 * note:
 *   ÓÉÓÚÊ±ŒäÔ­ÒòÔÝÊ±ÎŽµ÷ÊÔ£¬¶Ômore_rbsp_data()ÎŽÊµÏÖ¡£Ä¿Ç°PPSœâÎö²»¿ÉÓÃ
 ***********************************************************************/
int h264dec_picture_parameter_set(void *buf, int buflength, PPS *pps_ptr);
